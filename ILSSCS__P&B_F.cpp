...
// ************************************************** //
		  ...
		  out <<  "* Upper Bound: Relax-and-Fix procedure (product-oriented decomposition)" << endl;
...
//    with Product-Oriented Decomposition     //
// ****************************************** //
		 ...
		 //The Relax-and-Fix procedure considers a product-oriented decomposition
		 //For each product-window the whole planning horizon is considered. 
		 //The binary variables related to a cutting pattern is considered in the window 
		 //if it has a piece that belongs to the final products in the window. 
		 ...
		 int F_WindowSize,																			//number of periods in the product-window (F_WindowSize = F_Fix + F_Overlap),
			 
			 F_Fix,																					//number of periods in the product-window that are fixed 
		     
			 F_Overlap;													                            //number of periods in the product-window that overlap  
		 

		 int F_int_begin, F_int_end;																//interval of periods where the setup variables are integer
		 ...
		 // ****************************************************************************************
		 //Assigning the values to the Relax-and-Fix Procedure with a Time-Oriented Decompostion 
		 ...
		 if (F == 3) {
					    ...
					    //Note that F_WindowSize = F_Fix + F_Overlap
						F_WindowSize = 2;
					    F_Fix = 1;
					    F_Overlap = 1;
					    F_int_begin = 0;
						...
						out << "**************           with Product-Oriented Decomposition           **************" << endl;
						out << endl << endl << endl;
						out << "Parameters to F = " << F << ": product-window = " << F_WindowSize << endl;
		 ...
		 //Time available in each product-window
		 ...
		 //binary values in the Relax-and-Fix procedure
		 ...
		 //Cutting Patterns
 	     IloArray<IloArray<IloNumArray> > WJJ_fix(env, T);
		 for(t=0; t<T; t++){
		    WJJ_fix[t] = IloArray<IloNumArray> (env, J[t]); 
		    for(j=0; j<J[t]; j++){ 
		       WJJ_fix[t][j] = IloNumArray(env, J[t], 0, IloInfinity);								
			}
		 }


		 IloArray<IloNumArray> WJ_fix(env, T);
		 for(t=0; t<T; t++)
		    WJ_fix[t] = IloNumArray(env, J[t], 0, 1, ILOINT);		   
		 ...
		 //to their binary values in the Relax-and-Fix procedure
		 ...
		 //Cutting Patterns
    	 IloArray <IloArray<IloRangeArray> > RestFixWJJ(env, T);
		 for(t=0; t<T; t++){
		    RestFixWJJ[t] = IloArray<IloRangeArray>(env, J[t]);
		    for(j=0; j<J[t]; j++){
			   RestFixWJJ[t][j] = IloRangeArray(env, J[t]);
			}
		 }	


    	 IloArray<IloRangeArray> RestFixWJ(env, T); 
	     for(t=0; t<T; t++)
		    RestFixWJ[t] = IloRangeArray(env, J[t]);
		 ...
		 //The convWJJ_aux parameter check if the integrality of the binary variables WJJ_{tjl}
		 //and WJ_{tj} related to the cutting pattern j has been aready inserted in the problem.
		 //This is necessary because the cutting patterns are related with more than
		 //one piece and the piece can be a component of more than one final product
		 //(0-no; 1-yes)
		 IloArray<IloNumArray> convWJJ_aux(env, T);
		 for(t=0; t<T; t++)
		    convWJJ_aux[t] = IloNumArray(env, J[t], 0, 1, ILOINT);



		 //The RestFixWJJ_aux parameter check if the constraint that fix variables WJJ_{tjl}
		 //and WJ{tj} related to the cutting pattern j has been aready inserted in the problem. 
		 //This is necessary because the cutting patterns are related with more than 
		 //one piece and the piece can be a component of more than one final product
		 //(0-no; 1-yes)
		 IloArray<IloNumArray> RestFixWJJ_aux(env, T); 
	     for(t=0; t<T; t++)
		    RestFixWJJ_aux[t] = IloNumArray(env, J[t]);
		 ...
		 //in the first F_Overlap products of the product-window

		 //Converte the float variables to binary
		 //Final Products
		 for(t=0; t<T; t++)
		    for(f=F_int_begin; f<F_Overlap; f++){
			   IPmodel.add(IloConversion(env, YFF[t][f], ILOBOOL));	  
		       IPmodel.add(IloConversion(env, YF[t][f], ILOBOOL));
			}
		 ...
		 for(f=F_int_begin; f<F_Overlap; f++){
			...
			//to the final product in the product-window
            for(p=0; p<P; p++){
		       if (r[f][p] > 0) {
				                   //For each cutting pattern check if the 
								   //cutting pattern has the corresponding piece
							       for(t=0; t<T; t++){
				                      for(j=0; j<J[t]; j++){
									     if ((a[t][p][j] > 0) && (convWJJ_aux[t][j] == 0)) {

											                    //Add the integrality constraints to WJJ and WJ
																//variables to the corresponding cutting patterns 		
															    IPmodel.add(IloConversion(env, WJJ[t][j], ILOBOOL));
														        IPmodel.add(IloConversion(env, WJ[t][j], ILOBOOL));
														        

																//Check the integrality constraints added
																convWJJ_aux[t][j] = 1;
		 ...
		 for(; ;){
				  ...
				  //Consider the integrality of the binary variables 
			      //from (F_int_begin+F_Overlap) to F_int_end, that is,
				  //the integrality in those variables added in each window

		 
		          //Converte the float variables to binary
 				  for(t=0; t<T; t++)
					 for(f = (F_int_begin+F_Overlap); f<F_int_end; f++){
					    IPmodel.add(IloConversion(env, YF[t][f], ILOBOOL));
					    IPmodel.add(IloConversion(env, YFF[t][f], ILOBOOL));	  
					 }
				  ...
				  for(f = (F_int_begin+F_Overlap); f<F_int_end; f++){
					 ...
					 for(p=0; p<P; p++){
					    if (r[f][p] > 0) {
										    //For each cutting pattern check if  
										    //the cutting pattern has the corresponding piece
										    for(t=0; t<T; t++){
											   for(j=0; j<J[t]; j++){
												  if ((a[t][p][j] > 0) && (convWJJ_aux[t][j] == 0)) {
				  ...
				  //Add the integratlity of all the variables in
				  ...
				  if (IPcplex.getStatus() == 3) {
									...
									out << "      using a product-oriented decomposition      " << endl;	 
								    out << endl << endl;
								    out << "*************************************************************************************" << endl;
								    // *****************************************************************************************************
				  ...
				  //variables in the Relax-and-Fix procedure
				  //Final Products
				  for(t=0; t<T; t++)
				  ...
				  for(t=0; t<T; t++)
					 for(j=0; j<J[t]; j++)
						for(l=0; l<J[t]; l++)
						   WJJ_fix[t][j][l] = IPcplex.getValue(WJJ[t][j][l]);


				  for(t=0; t<T; t++)
					 for(j=0; j<J[t]; j++)
				        WJ_fix[t][j] = IPcplex.getValue(WJ[t][j]);
				  ...
				  //at their values in the Relax-and-Fix procedure
				  //Final Products
				  for(t=0; t<T; t++)
					 for(f=F_int_begin; f < (F_int_begin+F_Fix); f++)
					    for(q=0; q<F; q++){
				 ...
				 //Cutting Patterns
				  ...
				  for(f=F_int_begin; f < (F_int_begin+F_Fix); f++){
					 ...
					 for(p=0; p<P; p++){
						if (r[f][p] > 0) {
											//For each cutting pattern check if  
											//the cutting pattern has the corresponding piece and						
											//the constraint that fix the setup variables related
							                //to the cutting pattern (RestFixWJ_aux_tj) has not been added yet
											for(t=0; t<T; t++){
											   for(j=0; j<J[t]; j++){
												  if ((a[t][p][j] > 0) && (RestFixWJJ_aux[t][j] == 0)) {


																		 //Add the fix constraints to the WJJ_{tjl} variables
																	     for(l=0; l<J[t]; l++){
																		    IloExpr fix(env);
					      
																		    fix += WJJ[t][j][l];

																		    RestFixWJJ[t][j][l] = (fix == WJJ_fix[t][j][l]);
																		    RestFixWJJ[t][j][l].setName("FixWJJ");
																		    IPmodel.add(RestFixWJJ[t][j][l]);
					      
																		    fix.end();
																	     }

																		 
																		 //Add the fix constraints to the WJ_{tj} variables
																	     IloExpr fix(env);

																	     fix += WJ[t][j];
					   
																	     RestFixWJ[t][j] = (fix == WJ_fix[t][j]);
																	     RestFixWJ[t][j].setName("FixWJ");
																	     IPmodel.add(RestFixWJ[t][j]);
					   
																	     fix.end();


																		 
																		 //Check the fix constraints added
																	     RestFixWJJ_aux[t][j] = 1;
				  ...
				  //Move the product-window in the Relax-and-Fix procedure
				  ...
				  //Check if the last window of the Relax-and-Fix procedure is reached 
				  //if yes, the next iteration is the last iteration of the procedure
				  if (F_int_end >= F) {
										 F_int_end = F;
