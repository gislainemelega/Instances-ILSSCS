...
//  	         using Column Generation Procedure and an Optimization Package					   //
//																								   //
//																								   //
...
// *********************************** //
		  ...
		  //Constraint changed, due to periods with no production
...
// ************************************************** //
		  ...
		  out <<  "* Feasible Solution: Optimization Package " << endl;
		  ...
		  for(; ;){
				...
				for(t=0; t<T; t++){
						  ...
						  if (SUBcplex.getValue(SUBof) < -0.0001) {
													...
													//Add the ATSPJ5[t][J[t]-1] constraints in the restricted
													//master problem to the new cutting pattern (new constraints)
													if(t <= T-2) {
																	IloExpr atsp5(env);
...
// **************************************************************** // 
//    Solve the Integrated Problem using an Optimization Package    //
// **************************************************************** //


		  double time_ILSSCS;																		//time spent to solve the ILSSCS problem (integer problem)

		  double ZILSSCS, 																			//objective function value of the ILSSCS problem (integer problem)
				 gapILSSCS;																		    //gap of the ILSSCS problem (integer problem)
																									//considering the column generation solution as the lower bound



         

		  //The Integrated Lot-Sizing, Scheduling and Cutting Stock Problem (ILSSCS) 
		  //is solved by an optimization package considering all the columns obtained 
		  //with Column Generation Procedure and the integrality constraint of the variables

		  
		  //Converte the double variables to binary and integer variables
		  for(t=0; t<T; t++)
          ...
          // ***** Solve the Integrated Problem - ILSSCS ****************************************

		  //Add CPLEX Options
          //Print the output and warnings of cplex in the output file
          IPcplex.setOut(out);
          IPcplex.setWarning(out);


		  //Limite the number of threads in the solution of the Integrated Problem
		  IPcplex.setParam(IloCplex::Threads, 1);
		  ...
		  //Time Limit
		  IPcplex.setParam(IloCplex::TiLim, 1800);
		  ...
		  //Extract the ILSSCS problem
		  //IPcplex.extract(IPmodel);
		  //Export the GILSSCS problem
		  //IPcplex.exportModel ("ILSSCSmodel.lp" );
		  ...
		  out << "**************  Solving the ILSSCS problem by an Optimization Package  **************" << endl;
		  out << endl << endl;



		  //SOLVE the Integer Problem - ILSSCS
          IPcplex.solve();
		  ...
		  //Calcule the time spent to solve the ILSSCS problem  (integer problem)
		  time_ILSSCS = IPcplex.getTime() - time_CG;


		  
		  //Recover the objective function value of ILSSCS problem (integer problem)
		  ...
		  out << "Time ILSSCS = " << time_ILSSCS << endl;	 
		  out << endl << endl;
		  out << "*************************************************************************************" << endl;
		  // ****************************************************************************************************
