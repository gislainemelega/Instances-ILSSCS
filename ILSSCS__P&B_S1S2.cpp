...
// ****************************************** //
		 ...
		 //For each level-window the whole set of periods, final products and pieces are 
		 ...
		 //considers the level of cutting patterns (L2), which is modeled by
		 //the cutting stock problem and then the second window considers the 
		 //level of final products (L3), which is modeld by the lot-sizing problem
		 ...
		 out << endl << endl << endl;
         out << "* Level-oriented decomposition: 1) Cutting Patterns (L2 - Cutting Stock Problem)" << endl;
         out << "                                2) Final Products   (L3 - Lot-Sizing Problem)" << endl;
		 out << endl << endl << endl << endl << endl;
		 // ***************************************************************************************************** 
		 ...
		 //of the cutting patterns in the Relax-and-Fix procedure
		 ...
		 //Create the constraints that fix the binary variables of the cutting
		 //patterns to their binary values in the Relax-and-Fix procedure
		 ...
		 for(; ;){
				  ...
				  if (checkRF_lastWindow == 1) {
												  ...
												  //Add the integratlity of all the remain variables
												  //in the last iteration of the Relax-and-Fix procedure
												  for(t=0; t<T; t++)
				  ...
				  //Recover the values of the binary variables to the
				  //cutting patterns (L2), in the Relax-and-Fix procedure
				  for(t=0; t<T; t++)
 					 for(j=0; j<J[t]; j++)
						for(l=0; l<J[t]; l++)
				  ...
				  //Add the constraints that fix the binary variables to the cutting
				  //patterns (L2), at their values in the Relax-and-Fix procedure
				  for(t=0; t<T; t++)
				     for(j=0; j<J[t]; j++)
					   for(l=0; l<J[t]; l++){
					      IloExpr fix(env);
